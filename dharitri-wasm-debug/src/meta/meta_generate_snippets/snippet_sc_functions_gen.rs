...
use dharitri_wasm::abi::{ContractAbi, EndpointAbi, EndpointMutabilityAbi, InputAbi, OutputAbi};
...
fn write_deploy_method_impl(
    ...
    writeln!(
        file,
        r#"        let result: dharitri_interact_snippets::InteractorResult<PlaceholderOutput> = self
            .interactor
            .sc_deploy(
                self.contract
                    .{}({})
...
fn write_payments_declaration(file: &mut File, accepted_tokens: &[&str]) {
    ...
    if first_accepted == "MOA" {
        writeln!(file, "        let moa_amount = 0u64;").unwrap();
    } else {
        writeln!(
            file,
            "        let token_id = b\"\";
