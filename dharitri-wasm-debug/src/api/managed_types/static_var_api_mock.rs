...
impl StaticVarApiImpl for DebugApi {
    ...
    fn set_call_value_moa_handle(&self, handle: Self::BigIntHandle) {
        self.static_vars_cell.borrow_mut().call_value_moa_handle = handle.get_raw_handle();
    }

    fn get_call_value_moa_handle(&self) -> Self::BigIntHandle {
        use_raw_handle(self.static_vars_cell.borrow().call_value_moa_handle)
    }
