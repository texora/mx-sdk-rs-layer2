...
pub fn default_execution(tx_input: TxInput, tx_cache: TxCache) -> (TxResult, BlockchainUpdate) {
    ...
    tx_context.tx_cache.subtract_moa_balance(
        &tx_context.tx_input_box.from,
        &tx_context.tx_input_box.moa_value,
    );
    tx_context.tx_cache.increase_moa_balance(
    ...
    let add_transfer_log = is_smart_contract_address(&tx_context.tx_input_box.from)
        && !tx_context.tx_input_box.moa_value.is_zero();
    let transfer_value_log = if add_transfer_log {
        Some(TxLog {
            ...
            topics: vec![
                ...
                tx_context.tx_input_box.moa_value.to_bytes_be(),
            ],
            data: Vec::new(),
        })
    } else {
