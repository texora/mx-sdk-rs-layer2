...
use crate::multisig_rust_test_setup::{ActionRaw, MOA_TOKEN_ID};
...
fn transfer_execute_to_user_test() {
    ...
    ms_setup
        .call_deposit(MOA_TOKEN_ID, moa_amount)
        .assert_ok();

    ms_setup.b_mock.check_moa_balance(
        ms_setup.ms_wrapper.address_ref(),
        &rust_biguint!(moa_amount),
    );
    ...
    ms_setup
        .b_mock
        .check_moa_balance(&user_addr, &rust_biguint!(moa_amount));
}
