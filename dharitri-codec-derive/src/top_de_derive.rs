    ...
    match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                ...
                if !dharitri_codec::NestedDecodeInput::is_depleted(&nested_buffer) {
                    return core::result::Result::Err(h.handle_error(dharitri_codec::DecodeError::INPUT_TOO_LONG));
                }
        ...
        syn::Data::Enum(data_enum) => {
            ...
            if is_fieldless_enum(data_enum) {
                ...
                quote! {
                    match <u8 as dharitri_codec::TopDecode>::top_decode_or_handle_err(top_input, h)? {
                        #(#top_decode_arms)*
                        _ => core::result::Result::Err(h.handle_error(dharitri_codec::DecodeError::INVALID_VALUE)),
                    }
                }
            } else {
                ...
                quote! {
                    let mut nested_buffer = top_input.into_nested_buffer();
                    let result = match <u8 as dharitri_codec::NestedDecode>::dep_decode_or_handle_err(&mut nested_buffer, h)? {
                        #(#variant_dep_decode_snippets)*
                        _ => core::result::Result::Err(
                            h.handle_error(dharitri_codec::DecodeError::INVALID_VALUE),
                        ),
                    };
                    if !dharitri_codec::NestedDecodeInput::is_depleted(&nested_buffer) {
...
pub fn top_decode_or_default_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics dharitri_codec::TopDecode for #name #ty_generics #where_clause {
            ...
            {
                if top_input.byte_len() == 0 {
                    core::result::Result::Ok(<#name #ty_generics as dharitri_codec::DecodeDefault>::default())
                } else {
                    #top_decode_body
                }
            }
