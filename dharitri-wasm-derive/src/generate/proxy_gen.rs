...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut token_expr =
        quote! { dharitri_wasm::types::MoaOrDctTokenIdentifier::<Self::Api>::moa() };
        ...
        match &arg.metadata.payment {
            ArgPaymentMetadata::NotPayment => {
                ...
                arg_push_snippets.push(quote! {
                    dharitri_wasm::types::ContractCall::proxy_arg(&mut ___contract_call___, &#pat);
                });
            },
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            contract_call_type = quote! { dharitri_wasm::types::ContractCallWithMoa };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_wasm::types::ContractCallWithMoa::new(
                    ___address___,
                    #endpoint_name,
                    #payment_expr,
                );
            ...
            contract_call_type = quote! { dharitri_wasm::types::ContractCallWithMoaOrSingleDct };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_wasm::types::ContractCallWithMoaOrSingleDct::new(
                    ___address___,
                    #endpoint_name,
                    #token_expr,
                    #nonce_expr,
        ...
        contract_call_type = quote! { dharitri_wasm::types::ContractCallWithMultiDct };
        contract_call_init = quote! {
            let mut ___contract_call___ = dharitri_wasm::types::ContractCallWithMultiDct::new(
                ___address___,
                #endpoint_name,
                #multi_expr,
            );
        ...
        contract_call_type = quote! { dharitri_wasm::types::ContractCallNoPayment };
        contract_call_init = quote! {
            let mut ___contract_call___ = dharitri_wasm::types::ContractCallNoPayment::new(
                ___address___,
                #endpoint_name,
            );
        };
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    let msig =
        generate_proxy_method_sig(init_method, quote! { dharitri_wasm::types::ContractDeploy });
...
fn equivalent_encode_path_gen(ty: &syn::Type) -> syn::Path {
    ...
    syn::parse_str(
        format!(
            "dharitri_wasm::dharitri_codec::CodecInto<{}>",
