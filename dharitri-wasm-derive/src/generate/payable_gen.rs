...
fn call_value_init_snippet(mpm: MethodPayableMetadata) -> proc_macro2::TokenStream {
    match &mpm {
        MethodPayableMetadata::NotPayable => {
            quote! {
                dharitri_wasm::io::call_value_init::not_payable::<Self::Api>();
            }
        },
        MethodPayableMetadata::Moa => {
            quote! {
                dharitri_wasm::io::call_value_init::payable_moa::<Self::Api>();
            }
        },
        MethodPayableMetadata::SingleDctToken(token_identifier) => {
            quote! {
                dharitri_wasm::io::call_value_init::payable_single_specific_token::<Self::Api>(#token_identifier);
            }
        },
        MethodPayableMetadata::AnyToken => {
            quote! {
                dharitri_wasm::io::call_value_init::payable_any::<Self::Api>();
            }
        },
    }
}
...
fn opt_payment_arg_snippet(
    ...
    opt_arg
        ...
        .map(|arg| {
            let pat = &arg.pat;
            quote! {
                let #pat = dharitri_wasm::io::call_value_init::#init_fn_name::<Self::Api>();
            }
        })
        .unwrap_or_default()
}
